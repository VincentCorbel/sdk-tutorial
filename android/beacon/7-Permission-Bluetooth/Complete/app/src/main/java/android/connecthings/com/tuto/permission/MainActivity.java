...
public class MainActivity extends AppCompatActivity implements BeaconRange, View.OnClickListener{
    ...
    public void testToGrantBluetoothAccess(){
        if(!adtagBeaconManager.isBleAccessAuthorize()){
            ...
            //when giving authorization the ranging must be started manually.
            //Otherwise the ranging will start automically when the activity is resumed
            adtagBeaconManager.startRanging(this);
            switchToBeaconTv();
        }
    }
    ...
    private void testBluetoothPermission(){
        if(adtagBeaconManager.isBleAccessAuthorize()) {
            ...
            //Ble access has been ask before
            if(adtagBeaconManager.isBleAccessAsk()){
                showBluetoothTv();
                hideLocationTv();
            }else{
                //ble access has never been asked
                BleDialog.newInstance().show(getFragmentManager(), BleDialog.TAG);
            }
        }
    }
